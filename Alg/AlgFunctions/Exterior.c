...
int SetClosestPointsExterior(int idGroup, Group *closestPoints, Group *allGroups, unsigned int numGroups, unsigned short UNITSAPPROXIMATION){
...
int ExteriorAlgorithm(Group *externalPointsBoundaryGroup, Group buildingGroup, Group *allGroups, int numGroups, int k){
    ...
    Group *buildingBoundaryGroup, *externalPointsGroup, *closestPointsGroup, *closestPointsBoundaryGroup;
    Node aux, aux2;
    Lpoint p;
    ...
    removeGroup(externalPointsBoundaryGroup);
	...
	system("gcc -o Triangle/triangle Triangle/triangle.c -lm");
        ...
        //Search closest points to building points
         SetClosestPointsExterior(buildingGroup->id, closestPointsGroup, allGroups, numGroups, UNITSAPPROXIMATION);
         ...
         if(ConcaveAlgorithm(externalPointsBoundaryGroup, *externalPointsGroup,k) == EXIT_SUCCESS)
         {
            ...
            ExternalHeight = ExternalHeight /((*externalPointsGroup)->size);    
           ...
           (*externalPointsBoundaryGroup)->volume = ((*externalPointsBoundaryGroup)->area)*(InternalHeight-ExternalHeight);
